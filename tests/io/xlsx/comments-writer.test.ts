...
describe('XlsxWriter Classic Comments', () => {
    ...
    const createTestFile = async (): Promise<string> => {
        ...
        // Defensive: some platforms/filesystems can delay visibility of a freshly written file.
        for (let i = 0; i < 50; i += 1) {
            if (fs.existsSync(testFile) && fs.statSync(testFile).size > 0) return testFile;
            await new Promise<void>(resolve => setTimeout(resolve, 10));
        }

        throw new Error(`Expected XLSX file to exist after save: ${testFile}`);
    };
    ...
    test(
        'writes required parts and relationships',
        async () => {
    ...
    test(
        'XlsxReader loads classic comments unless readDataOnly',
        async () => {
            const testFile = await createTestFile();
