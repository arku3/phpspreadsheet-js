...
export class Engineering implements FunctionCategory {
    public register(registry: FunctionRegistry): void {
        ...
        registry.register('COMPLEX', (args) => {
        ...
        registry.register('IMPRODUCT', (args) => {
            if (args.length < 2) return '#VALUE!';
            const numbers = args.map(arg => String(Helpers.asScalar(arg) || ''));
        ...
        registry.register('CONVERT', (args) => {
        ...
        registry.register('DEC2BIN', (args) => {
        ...
        registry.register('HEX2DEC', (args) => {
            const number = String(Helpers.asScalar(args[0]) || '');
    ...
    private parseComplex(complex: string): { real: number; imag: number } | null {
        ...
        if (!match) {
            ...
            if (simpleImag) {
                const imag = simpleImag[1] === '' || simpleImag[1] === '+' ? 1 : 
                            simpleImag[1] === '-' ? -1 : Number(simpleImag[1]);
                return { real: 0, imag };
            }
