...
- **Concurrent Safety:** When instructing subagents, explicitly tell them to avoid undoing, removing, reformatting, or otherwise modifying files that are outside their assigned scope (these may be concurrent work from other subagents; they must not interfere with each other).
