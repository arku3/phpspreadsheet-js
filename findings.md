...
- Table XLSX writer parity improved: `src/io/xlsx/table.ts` now serializes table-level `<autoFilter>` metadata like PhpSpreadsheet, including hidden filter buttons from `TableColumn.showFilterButton` plus value/custom/top10 rule payloads from the table's own `AutoFilter` model.
- Table-level AutoFilter writer coverage now also locks blank filters, date-group filters, and dynamic filters in `xl/tables/tableN.xml`, including `blank="1"`, `dateGroupItem ... dateTimeGrouping="..."`, and dynamic `type/val/maxVal` attributes.
...
- Worksheet/table API parity improved further: `Worksheet` now exposes PHP-like table collection helpers (`getTableCollection`, case-insensitive `getTableByName`, chainable `removeTableByName`, chainable `removeTableCollection`), and `Table` mutators now follow PHP behavior more closely (`setColumn(string|TableColumn)`, no-op `clearColumn` for unset in-range columns, no-op `shiftColumn` for missing sources, and detach/rebind semantics when shifting existing columns).
...
- Conditional-formatting XLSX writing now includes PHP-like `timePeriod` serialization in `src/io/xlsx/worksheet.ts`: the writer emits `timePeriod="..."`, generates the same default formulas for all supported periods (`today` through `nextMonth`), preserves explicit formulas when provided, and normalizes non-cell sqref anchors like `D:D` to `D1`.
- Added `src/io/xlsx/function-prefix.ts` and wired conditional-formatting formula writing through it so saved XLSX files now prefix modern Excel functions like PhpSpreadsheet (`FILTER` -> `_xlfn._xlws.FILTER`, spill refs like `A1#` -> `_xlfn.ANCHORARRAY(A1)`) without double-prefixing already-prefixed formulas. Conditional writer formulas now also serialize boolean conditions as `TRUE` / `FALSE` instead of JS lowercase literals.
- `ConditionalIconSet` now matches PHP more closely by preserving only explicitly provided cfvo thresholds instead of auto-generating defaults when the icon-set type is set, and the worksheet writer now emits `custom` plus explicit `gte` flags for icon-set thresholds.
- `Worksheet` conditional-style APIs now match PhpSpreadsheet more closely: single-cell `getConditionalStyles(...)` searches inside stored ranges/unions/intersections, `getConditionalRange(...)` returns the original matched range string, flattened multi-match lookups sort by priority with `0` last, and `duplicateConditionalStyle(...)` now exists for copying conditional arrays across a target range.
- Added focused worksheet parity coverage in `tests/core/worksheet-conditional-styles-parity.test.ts` for in-range lookup, intersection-aware ranges, priority ordering, and exact-key removal semantics.
- Conditional color-scale runtime matching now treats numeric strings like PHP `is_numeric`, both when deciding whether a color-scale rule applies and when coercing sqref values into min/max calculations. Mixed string/text inputs no longer poison the scale with `NaN`, and numeric-string cells now receive color-scale fills through `CellStyleAssessor`.
- `Cell.getAppliedStyle()` now mirrors PhpSpreadsheet by routing through worksheet conditional-style lookup and `CellStyleAssessor` instead of always returning the base style. This makes matched font and color-scale conditional formatting visible through the public cell API, including `stopIfTrue` and intersection-aware conditional ranges.
- `CellMatcher.wrapValue()` now treats numeric strings like PHP `is_numeric` rather than always quoting them, and duplicate/unique runtime checks now build their `COUNTIF` criterion from `getCalculatedValueString()` passed through `cellConditionCheck(...)`. This aligns `cellIs`, range operators, duplicate/unique rules, and `Cell.getAppliedStyle()` for explicit string numerics such as `'7'` and `'10'`.
- Added `MergedCellStyle` parity for combined table-plus-conditional applied styles. `Worksheet.addTable()` now binds tables back to the worksheet and returns the worksheet, new helpers `getTablesWithStylesForCell()` / `getTablesWithoutStylesForCell()` mirror PHP table-style lookup, and merged style resolution now applies custom table dxfs stripes, built-in fallback table fills, and only the first matched conditional overlay.
- `Spreadsheet.copy()` now preserves worksheet tables instead of dropping them: copied sheets receive cloned table objects rebound to the copied worksheet, table AutoFilter columns/rules/attributes survive the copy, table column metadata and formulas are preserved, and cloned `TableStyle` / `TableDxfsStyle` state keeps merged table styling resolvable after copy and subsequent XLSX writing.
- Table and AutoFilter range parsing now mirrors PhpSpreadsheet more closely: shared `Worksheet.extractSheetTitle()` / `unApostrophizeTitle()` handle quoted sheet titles and doubled apostrophes, `AutoFilter` strips sheet-qualified and `$`-absolute prefixes before storing normalized ranges, and `Table` accepts numeric array ranges like `[1, 1, 3, 4]` while pruning out-of-range columns without auto-creating default column objects.
